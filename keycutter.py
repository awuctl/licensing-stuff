...
class ProductKey2009:
    ...
    class KeyParams:
        ...
        group:    int
        """
        The Group generally specifies the product the key is for.
        This number is unique across all Microsoft products (Office, Windows, Visual Studio) and even key algorithms.

        Maximum value: 0xfffff (20 bits)
        """

        serial:   int
        """
        The Serial is a number that differentiates product keys in a given group.
        It grows sequentially, within a given (PKeyConfig) range.

        Sometimes, different ranges are used for subtypes in licensing.
        eg. some key ranges of Pro were sold as Pro Education before it became its own edition.

        Maximum value: 0x3fffffff (30 bits)
        """
        
        security: int
        """
        The Security value is the parameter responsible for the validity of the group-serial-upgrade combo.

        There is a possibility this is actually two fields (see patent US8984293B2), 38 and 15 bits each.
        PKeyConfigs hint that this is some form of a SHA256 HMAC.
        
        Maximum value: 0x1fffffffffffff (53 bits)
        """
        
        checksum: int
        """
        The Checksum is a checksum of the whole key.

        It's the least significant 10 bits of a CRC32/MPEG-2 of the key expanded to 128 bits.

        Maximum value: 0x3ff (10 bits)
        """
        
        upgrade:  int
        """
        The Upgrade bit specifies whether or not the key is an upgrade key.

        Maximum value: 0x1 (1 bit)
        """
        
        extra:    int
        """
        The Extra bit has no discernible meaning, it's just sometimes encodable.

        This very often allows for the creation of two different keys with the same meaning.
        Unsurprisingly, both will work for regular activation, but they can behave weirdly with Modern licensing.

        As long as the upgrade bit is not set, it is definitely encodable.
        Otherwise, the requirement is: int(key) <= 0x00062A32B15518000000000000000000 (pow(24, 25))
        """

        # for making sure the fields are valid
        def _ensure_ranges(func):
            """Decorates function to make sure the input's fields are within bounds."""
            def check(*args, **kwargs):
                grp = args[0].group    ; ser = args[0].serial
                sec = args[0].security ; chk = args[0].checksum
                upg = args[0].upgrade  ; ext = args[0].extra

                if False in ([val <= bound for val,bound in zip(
                    [grp,     ser,        sec,              chk,   upg, ext],
                    [0xfffff, 0x3fffffff, 0x1fffffffffffff, 0x3ff, 0x1, 0x1]
                )]):
                    return func(*args, **kwargs)
                else:
                    raise Exception('Key field bounds check not passed')
            return check
        ...
        def __init__(
            self,
            calc_checksum: bool,
            key: Union[int, Tuple[int,int,int,int,int,int]] = 0,
        ):
            """
            Creates a key object from its parameters (key=Tuple) or numeric value (key=int)
            calc_checksum: determines whether
            """
